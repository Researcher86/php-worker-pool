...
- [Four holes a review found at the edges](#four-holes-a-review-found-at-the-edges)
...
| Clients may send `REQUEST` frames and nothing else; the Master assigns the type, as it does the id | current, [why](#four-holes-a-review-found-at-the-edges) |
| A client that can no longer be written to is dropped, not merely muted | current, [why](#four-holes-a-review-found-at-the-edges) |
| Leaving the pool is bounded too (`workerDepartureTimeoutSeconds`), not just working and starting | current, [why](#four-holes-a-review-found-at-the-edges) |
| The five request counters are a partition of `requestsTotal`, asserted as one | current, [why](#four-holes-a-review-found-at-the-edges) |
| **A drained worker is exempt from the execution limit** | **reversed** - [why](#four-holes-a-review-found-at-the-edges). It kept its full budget and then some: past the limit, nothing in the pool could ever end it. |
...
# Four holes a review found at the edges

An outside read of the whole codebase, rather than a failing test, is what
produced these. That is the part worth recording: in all four, the absence
of a rule looked exactly like a rule being obeyed. Nothing crashed, no test
went red, and each one had a docblock nearby explaining a design that was
almost right.

## A client could end a worker

Both ends of the pool speak one wire format, and `Master::handleClientRequest`
forwarded the client's frame type into the pool verbatim:

```php
$this->dispatcher->dispatch(new Message($request->type, $dispatchId, $request->payload));
```

`MessageDecoder` accepts any `MessageType` the enum defines, so a client
that sent `{"type":"shutdown"}` had it delivered to whichever worker was
next in rotation - and `WorkerRunner::run()` treats SHUTDOWN as exactly what
it says: `close(); return;`. The worker exited as if the Master had retired
it, its in-flight request came back `worker_crashed`, and the pool paid a
reap and a fork. One frame per worker, on repeat, from a peer whose only
privilege is being able to ask for work. On the `0660 + shared group` shape
[recommended for production](FAILURE-MODEL.md#who-may-connect), that is every
account in the group.

The fix is one guard, and its shape is the interesting part: the Master
already refuses to trust the client's *id* (`PendingRequestRegistry` assigns
its own, so two clients cannot misroute each other's responses). The type
belongs in the same category - a field the protocol lets the client set, and
the Master has no reason to honour. Only `REQUEST` is accepted now.

The guard is deliberately the *only* place that knows this rule; the
dispatch still forwards `$request->type` rather than restating `REQUEST`.
The day a client is allowed to send some second type, one condition loosens
instead of two places having to agree.

Rejected: answering `unsupported_message_type` and keeping the connection.
A peer sending frames no client may send is not a peer with a bad request,
it is a peer that is not speaking this protocol - and every other instance
of that (unparseable JSON, a framing desync) already costs the connection.
Keeping it also leaves the churn loop intact, just one indirection later.

## A client we could no longer answer kept its requests running

`ClientConnection` gives up on a peer that has stopped reading once its
unsent responses pass 4 MiB - `markBroken()` drops the buffer and
half-closes the socket, so nothing is retained and nothing further is sent.
What it did not do is remove the client: it kept its read handler in
`ClientRegistry`, so it could go on submitting requests indefinitely, each
one taking a queue slot and a worker to produce an answer that could never
be delivered. The comment on the flag said "the read side is what
ultimately removes the client" - true only if the client ever disconnects,
which is precisely what this client is not doing.

Being unable to answer a client is the point at which its requests stop
being worth running, so it is now dropped: `isBroken()` plus a
`removeBroken()` sweep Master runs once per tick, next to its other sweeps.

Polled rather than pushed, deliberately. Breaking happens deep inside a
write, and a write is often reached from `ClientRegistry`'s own read handler
- mid-iteration over that same client's decoded batch. Removing it from
there would close the socket underneath the loop still reading it. A sweep
at a known-safe point in the tick costs one comparison per client and has no
such window. The one thing the sweep cannot cover - the rest of a batch
decoded in the same read - is a `isBroken()` check in the read loop, which
is where the break becomes visible first.

## Leaving the pool had no deadline

Every other way a worker occupies a slot is bounded. A request has the
execution limit, a bootstrap has its own, a crash is reaped and replaced
within a tick. Leaving had nothing: `terminateStuckWorkers()` skipped
draining workers outright, and a retired one (STOPPING, SHUTDOWN sent,
socket closed) was skipped by every sweep there is. A worker that ignores
its shutdown - or a drained one whose handler never returns - held its slot,
its pid and its telemetry slot for as long as the Master lived, and nothing
anywhere would ever ask why.

`workerDepartureTimeoutSeconds` (10s) is that bound: told to leave, and
still here. Two details make it safe rather than merely strict:

- **A drained worker still keeps its whole execution budget.** "Draining
  never interrupts a request" is unchanged; what changed is that the
  exemption now ends where its own justification does. "It will leave on its
  own" is exactly the claim a request 61 seconds into a 60-second limit has
  already disproved.
- **A leaving worker is stopped before it is signalled.** The reaper reads a
  STOPPING exit as expected: no crash counted, and no replacement - because
  whoever drained it launched one already. Killing it as a crash instead
  would fork a second replacement and leave the pool one worker over its
  ceiling, which is [invariant 5](FAILURE-MODEL.md#what-is-actually-guaranteed).

Its in-flight request, if it somehow still has one, is abandoned without a
synthesized error - by the time an execution limit set above the request
timeout fires, whoever was waiting has already been told `request_timeout`.

## The request counters were four numbers, not a partition

`requestsFailed`, `requestsTimeout` and `requestsRejected` are counted in
three components that have no view of each other (`RequestMetrics`,
`PendingRequestRegistry`, `RequestQueue`), and their sum never equalled
`requestsTotal` - not because anything was miscounted, but because the term
that closes the identity was missing from the snapshot: the requests still
in flight.

`requestsPending` is now in `Metrics` (and in the SIGUSR1 dump, where an
in-flight count is worth reading anyway), and `requestsAccountedFor()`
states the identity in one place: **answered + failed + timed out + rejected
+ in flight = accepted**, in every snapshot, at any moment. It is asserted
twice - once as arithmetic over each of Master's five accounting paths
(`MetricsCollectorTest`), once against a real Master's own dump under a
mixed workload (`InvariantsTest`, invariant 8). A counter that drifts is
otherwise the kind of thing noticed six months later, in production, by
someone who cannot tell whether the number or the pool is wrong.

## One chore, while in there

`composer analyse` now passes `--memory-limit=1G`. PHPStan's parallel
