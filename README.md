...
| a worker warms up before it is given any work | [`Worker/Runtime/WorkerRunner.php`](src/Worker/Runtime/WorkerRunner.php) |
...
| a worker reports what only it can measure about itself, lock-free | [`Worker/Telemetry/SharedTelemetry.php`](src/Worker/Telemetry/SharedTelemetry.php) |
...
| a worker loop stays alive through a handler that throws | [`Worker/Runtime/WorkerRunner.php`](src/Worker/Runtime/WorkerRunner.php) |
...
The layout below is the one sketched **before** any of this was built, kept
here with the rest of the plan. It is not what the code looks like today -
`Master/EventLoop.php`, `Protocol/LengthPrefixedProtocol.php`, `Request/`
and `Support/IdGenerator.php` never existed, and `Worker/` has since been
split along the fork into `Worker/Runtime/` and `Worker/Telemetry/`.

For the structure as it actually is, and why each piece ended up where it
did, see
[Where the code ended up, and why](docs/DECISIONS.md#where-the-code-ended-up-and-why)
and [Where Worker/ was split, and where it wasn't](docs/DECISIONS.md#where-worker-was-split-and-where-it-wasnt).
